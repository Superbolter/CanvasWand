                ...
                >
                    {
                        this.props.filter.data.map((data) => {
                            ...
                            return <MenuItem onClick={() => this.handleOnClick(isActive, data.start_key, data.end_key, data.name)} style={{ dispatch: "flex", alignItems: "center", justifyContent: "flex-start" }}><Checkbox checked={isActive} style={{ padding: "0px", marginRight: "8px" }} />{data.name}</MenuItem>
                        }
                        )
                    }
                </Menu>
