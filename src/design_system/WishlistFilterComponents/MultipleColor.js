                ...
                <Collapse in={this.state.showFilters}>
                    {
                        this.props.filter.data.map((data) => {
                            ...
                            return <Typography  onClick={() => this.handleOnClick(isActive, data.key, data.name)} className={"multiple-select-item" + (isActive ? " filter-active" : "")} as="p" modifiers={["subtitle"]}><Checkbox checked={isActive} style={{ padding: "0px", marginRight: "8px" }} color="primary" /><span style={{ width: "24px", height: "24px", borderRadius: "50%", marginRight: "8px", background: data.key }}></span> {data.name}</Typography>
                        }
                        )
                    }
                </Collapse>
