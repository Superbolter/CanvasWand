                ...
                >
                    {
                        this.props.filter.data.map((data, i) => {
                            ...
                            return <MenuItem key={i} onClick={() => this.handleOnClick(isActive, data.key, data.name)} style={{ dispatch: "flex", alignItems: "center", justifyContent: "flex-start" }}><Checkbox checked={isActive} style={{ padding: "0px", marginRight: "8px" }} />{data.name}</MenuItem>
                        }
                        )
                    }
                </Menu>
