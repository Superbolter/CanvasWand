                ...
                <Collapse in={this.state.showFilters}>
                    {
                        this.props.filter.data.map((data, index) => {
                            ...
                            return <Typography key={index} onClick={() => this.handleOnClick(isActive, data.start_key, data.end_key, data.name)} className={"multiple-select-item" + (isActive ? " filter-active" : "")} modifiers={["subtitle"]}><Checkbox checked={isActive} style={{ padding: "0px", marginRight: "8px" }} color="primary" /> {data.name}</Typography>
                        }
